...
(function (bc19) {
    var Helper = (function () {
        ...
        Helper.ListenForBattleCry = function (robot) {
            var robots = robot.getVisibleRobots();
            for (var i = 0; i < robots.length; i++) {
                var enemyCastle = Helper.DecodeBattleCry(robot, robots[i].signal);
                if (enemyCastle != null) {
                    if (robot.me.unit !== robot.SPECS.PROPHET) {
                        return enemyCastle;
                    }
                    else if (Helper.ProphetBattleCry(robot, robots[i].signal)) {
                        return enemyCastle;
                    }
                }
            }
        ...
        Helper.ProphetBattleCry = function (robot, signal) {
            if (signal <= 20479 && signal >= 16384) {
                return true;
            }
            return false;
        };
        Helper.DecodeBattleCry = function (robot, signal) {
        ...
        Helper.BetweenTwoUnorderedNumbers = function (point, pos1, pos2) {
            var vector1to2 = new bc19.Position(pos2.y - pos1.y, pos2.x - pos1.x);
            var vector2to1 = new bc19.Position(pos1.y - pos2.y, pos1.x - pos2.x);
            var onePerpVec = new bc19.Position(vector1to2.x, vector2to1.y * -1);
            var twoPerpVec = new bc19.Position(vector2to1.x, vector2to1.y * -1);
            var pos1SecP = new bc19.Position(pos1.y + onePerpVec.y, pos1.x + onePerpVec.y);
            var pos2SecP = new bc19.Position(pos2.y + twoPerpVec.y, pos2.x + twoPerpVec.y);
            if (!Helper.leftOfLine(point, pos1, pos1SecP) && Helper.leftOfLine(point, pos2, pos2SecP)) {
                return true;
            }
            return false;
        };
        Helper.leftOfLine = function (point, pos1, pos2) {
...
(function (bc19) {
    var Church = (function (_super) {
        ...
        Church.prototype.Execute = function () {
            ...
            output = this.EvaluateEnemyRatio(this.robot);
            return output;
        };
