    ...
    var Pilgrim = (function (_super) {
        ...
        Pilgrim.prototype.ReturnToDropOff = function () {
            ...
            if (this.GetOrCreateMap(this.robot, this.allRoutes, dropOff, true)[this.robot.location.y][this.robot.location.x] < 25) {
                if (bc19.Helper.DistanceSquared(this.robot.location, dropOff) <= 16) {
                    ...
                    return bc19.MovingRobot.FloodPathing(this.robot, this.GetOrCreateMap(this.robot, this.allRoutes, dropOff, false), dropOff, false);
                }
            }
            return this.Mining();
        };
            ...
            if (this.depotNum >= 0) {
                ...
                if (bc19.Helper.DistanceSquared(this.robot.location, this.churchLocation) <= 16) {
                    if (this.robot.getKarboniteMap()[this.robot.me.y][this.robot.me.x] || this.robot.getFuelMap()[this.robot.me.y][this.robot.me.x]) {
                        ...
                        return bc19.MovingRobot.FloodPathing(this.robot, bc19.MovingRobot.CreateLayeredFloodPath$bc19_MyRobot$bc19_Position$bc19_Position(this.robot, nearestToChurch, this.robot.location), nearestInGeneral, true);
                    }
                }
                else {
                    return bc19.MovingRobot.FloodPathing(this.robot, bc19.MovingRobot.CreateLayeredFloodPath$bc19_MyRobot$bc19_Position$bc19_Position(this.robot, this.churchLocation, this.robot.location), this.churchLocation, false);
                }
            }
            else {
                if (this.robot.getKarboniteMap()[this.robot.me.y][this.robot.me.x] || this.robot.getFuelMap()[this.robot.me.y][this.robot.me.x]) {
                    ...
                    return bc19.MovingRobot.FloodPathing(this.robot, bc19.MovingRobot.CreateLayeredFloodPath$bc19_MyRobot$bc19_Position$bc19_Position(this.robot, nearestInGeneral, this.robot.location), nearestInGeneral, false);
            }
        };
        ...
        Pilgrim.ChurchBuildPosition = function (robot, pos) {
            ...
            for (var i = -1; i <= 1; i++) {
                for (var j = -1; j <= 1; j++) {
                    var adjacent = new bc19.Position(pos.y + i, pos.x + j);
                    if (bc19.Helper.TileEmptyNonResource(robot, adjacent)) {
                        var count = 0;
                        for (var k = -1; k <= 1; k++) {
                            for (var l = -1; l <= 1; l++) {
...
(function (bc19) {
    var Prophet = (function (_super) {
        ...
        Prophet.prototype.InitializeVariables = function () {
            ...
            this.enemyCastleLocations = ([]);
            this.routesToEnemies = ({});
            this.parentLocation = new bc19.Position(1, 1);
        };
...
(function (bc19) {
    var Castle = (function (_super) {
        ...
        Castle.prototype.InitializeVariables = function () {
            this.spawnOrder = [this.robot.SPECS.PROPHET, this.robot.SPECS.PROPHET];
