...
(function (bc19) {
    var Helper = (function () {
        ...
        Helper.UpdateBattleStatus = function (robot, enemies, enemy) {
            var battleCry = Helper.ListenForBattleCry(robot);
            if (battleCry != null) {
        ...
        Helper.ListenForBattleCry = function (robot) {
            var robots = robot.getVisibleRobots();
            for (var i = 0; i < robots.length; i++) {
                var enemyCastle = Helper.DecodeBattleCry(robots[i].signal);
                if (enemyCastle != null) {
                    if (robot.me.unit !== robot.SPECS.PROPHET) {
                        return enemyCastle;
                    }
                    else if (Helper.ProphetBattleCry(robots[i].signal)) {
                        return enemyCastle;
                    }
                }
            }
        ...
        Helper.ProphetBattleCry = function (signal) {
            if (signal < 61440 && signal >= 45056) {
                return true;
            }
            return false;
        };
        Helper.DecodeBattleCry = function (signal) {
            if (signal < 45056) {
                return null;
            }
            var x = signal & 63;
    ...
    var Crusader = (function (_super) {
        ...
        Crusader.prototype.Execute = function () {
            ...
            if (this.robot.me.turn === 1) {
                ...
                /* add */ (this.enemyCastleLocations.push(bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.parentLocation)) > 0);
            ...
            if (bc19.Helper.EnemiesAround(this.robot) && this.robot.fuel > 110) {
            ...
            for (var i = 0; i < this.robot.getVisibleRobots().length; i++) {
                if (this.robot.getVisibleRobots()[i].unit === this.robot.SPECS.CRUSADER) {
                    ++count;
                }
            }
            ;
            ...
            if (this.ready === true) {
                return bc19.MovingRobot.FloodPathing(this.robot, bc19.MovingRobot.CreateLayeredFloodPath$bc19_MyRobot$bc19_Position(this.robot, closestEnemyCastle), closestEnemyCastle, false);
            }
            return null;
        };
...
(function (bc19) {
    var Preacher = (function (_super) {
        ...
        Preacher.prototype.CastleInit = function () {
            ...
            this.initialized = signals[0];
            if (this.castleLocations.length > 0) {
                this.initialized = true;
            }
            if (this.initialized) {
...
(function (bc19) {
    var Prophet = (function (_super) {
        ...
        Prophet.prototype.Execute = function () {
            ...
            this.targetCastle = bc19.Helper.UpdateBattleStatus(this.robot, this.enemyCastleLocations, this.targetCastle);
            if (bc19.Helper.EnemiesAround(this.robot)) {
                var closeEnemies = bc19.Helper.EnemiesWithin(this.robot, this.robot.attackRange[0]);
                if (this.initialized && closeEnemies.length > 0 && this.robot.fuel > 200) {
            ...
            if (this.initialized && this.robot.fuel > 200) {
...
(function (bc19) {
    var Castle = (function (_super) {
        ...
        Castle.prototype.Execute = function () {
            ...
            this.robot.log("init problem?");
            if (this.initialized) {
                var random = bc19.Helper.RandomAdjacent(this.robot, new bc19.Position(this.robot.me.y, this.robot.me.x));
                for (var i = 0; i < this.robot.getVisibleRobots().length; i++) {
                    if (this.robot.getVisibleRobots()[i].castle_talk !== 0 && this.robot.getVisibleRobots()[i].id !== this.robot.me.id) {
                        return null;
                    }
                }
                ;
                this.robot.castleTalk(50);
                this.DeclareAllyCastlePositions(false, false);
                return this.robot.buildUnit(this.robot.SPECS.CRUSADER, random.x - this.robot.me.x, random.y - this.robot.me.y);
            }
            return null;
        };
        Castle.prototype.Initialize = function () {
            ...
            this.robot.log("initing variables");
            if (!this.initialized) {
        ...
        Castle.prototype.SignalAttack = function () {
            var otherCastlesCry = bc19.Helper.ListenForBattleCry(this.robot);
            ...
            if (otherCastlesCry == null && this.robot.me.turn % 200 === 0) {
                this.robot.log("Signalling " + (this.robot.me.turn === 200));
                this.robot.log("list size " + (function (a) { return a ? '[' + a.join(', ') + ']' : 'null'; })(this.allyCastlePositions) + " " + this.targetCastleIndex);
                ...
                this.robot.log("The Position is : " + enemyCastle);
                this.robot.signal(this.CreateAttackSignal(enemyCastle, this.robot.me.turn === 200), this.robot.map.length * this.robot.map.length + this.robot.map.length * this.robot.map.length);
                this.robot.log("sent the signal");
            }
            else if (this.robot.me.turn % 100 === 0) {
                this.targetCastleIndex++;
            }
        };
        Castle.prototype.CreateAttackSignal = function (pos, finalAttack) {
            var output = finalAttack ? 11 : 15;
            output <<= 6;
            output += pos.y;
            output <<= 6;
            output += pos.x;
