        ...
        MovingRobot.prototype.GetValidDefense = function (robot, routesToEnemy, parent, invader) {
            ...
            for (var y = -7; y <= 7; y++) {
                for (var x = -7; x <= 7; x++) {
                    ...
                    if (bc19.Helper.BetweenTwoPoints(robot, possible, parent, invader) && bc19.Helper.DistanceSquared(robot.location, possible) <= 49 && bc19.Helper.inMap(robot.map, possible)) {
            ...
            if (this.initialized && bc19.Helper.Have(this.robot, 0, 325)) {
                if (this.targetCastle == null && !this.Fortified(this.robot, this.parentLocation)) {
                    ...
                    if (valid.length > 0) {
                        ...
                        return bc19.MovingRobot.FloodPathing(this.robot, shortPath, towardsCenter, false);
                    }
                }
...
(function (bc19) {
    var Pilgrim = (function (_super) {
        ...
        Pilgrim.prototype.Execute = function () {
            ...
            if (this.initialized) {
                this.haveAChurch = this.ChurchLost();
        ...
        Pilgrim.prototype.ReturnToDropOff = function () {
            ...
            if (bc19.Helper.DistanceSquared(this.robot.location, dropOff.pos) <= 10) {
                return bc19.MovingRobot.MoveCloser(this.robot, dropOff.pos, this.haveAChurch ? true : false);
            }
            else if (this.spawnLocation.map[this.robot.location.y][this.robot.location.x] < 25) {
                return bc19.MovingRobot.FloodPathing(this.robot, dropOff.map, dropOff.pos, this.haveAChurch ? true : false);
            }
            else {
                return this.Mining();
            }
        ...
        Pilgrim.prototype.GetNearestResource = function () {
            ...
            for (var i = 0; i < chosenLocations.length; i++) {
                ...
                if (bc19.Helper.RobotAtPosition(this.robot, pos) == null && this.ImTheClosestPilgrim(pos) && distance < lowest) {
                    lowest = distance;
                    closest = pos;
                }
            }
        ...
        Pilgrim.prototype.GoToMine = function () {
            if (bc19.Helper.DistanceSquared(this.robot.location, this.myChurch.pos) <= 9) {
                if (this.robot.getKarboniteMap()[this.robot.me.y][this.robot.me.x] || this.robot.getFuelMap()[this.robot.me.y][this.robot.me.x]) {
                    ...
                    return bc19.MovingRobot.MoveCloser(this.robot, nearest, this.haveAChurch ? true : false);
                }
            }
            else {
                return bc19.MovingRobot.FloodPathing(this.robot, this.myChurch.map, this.myChurch.pos, this.haveAChurch ? true : false);
            }
        };
        ...
        Pilgrim.prototype.ChurchLost = function () {
            var robots = this.robot.getVisibleRobots();
            for (var i = 0; i < robots.length; i++) {
                if (robots[i].unit === this.robot.SPECS.CHURCH && robots[i].team === this.robot.ourTeam) {
                    return true;
                }
            }
            ;
    ...
    var Castle = (function (_super) {
        ...
        Castle.prototype.Execute = function () {
            ...
            if (resources > pilgrims) {
                var buildHere = bc19.Helper.RandomAdjacentNonResource(this.robot, this.robot.location);
                if (buildHere != null && bc19.Helper.Have(this.robot, 80, 125) || this.starterPilgrims < 4) {
            ...
            else if (bc19.Helper.Have(this.robot, 80, 125) || this.starterPilgrims < 4) {
                this.UpdateDepots();
...
(function (bc19) {
    var Church = (function (_super) {
        ...
        Church.prototype.Execute = function () {
            if (this.robot.me.turn === 1) {
                this.SetDepotNumer();
                this.spawnOrder = [this.robot.SPECS.CRUSADER, this.robot.SPECS.CRUSADER, this.robot.SPECS.PREACHER, this.robot.SPECS.PROPHET];
            }
            var output = null;
            var signal = -1;
            this.positionInSpawnOrder = this.positionInSpawnOrder === this.spawnOrder.length ? 0 : this.positionInSpawnOrder;
            ...
            if (!bc19.Helper.EnemiesAround(this.robot) && resources > pilgrims) {
                var buildHere = bc19.Helper.RandomAdjacentNonResource(this.robot, this.robot.location);
                if (buildHere != null && bc19.Helper.Have(this.robot, 80, 100)) {
                    signal = this.depotNum;
                    output = this.robot.buildUnit(this.robot.SPECS.PILGRIM, buildHere.x - this.robot.me.x, buildHere.y - this.robot.me.y);
                }
            }
