...
(function (bc19) {
    var MovingRobot = (function () {
        ...
        MovingRobot.CreateLayeredFloodPath$bc19_MyRobot$bc19_Position = function (robot, startPos) {
            return MovingRobot.CreateLayeredFloodPath$bc19_MyRobot$bc19_Position$bc19_Position(robot, startPos, new bc19.Position(500, 500));
        };
        ...
        MovingRobot.FloodPathing = function (robot, path, goal, budget, enemies) {
            if (path == null) {
                return null;
            }
            ...
            if (bc19.Helper.DistanceSquared(robot.location, goal) <= robot.movementRange) {
                ...
                if (bc19.Helper.TileEmpty(robot, goal)) {
                    for (var i = 0; i < enemies.length; i++) {
                        var unit = new bc19.Position(/* get */ enemies[i].y, /* get */ enemies[i].x);
                        if (bc19.Helper.DistanceSquared(goal, unit) <= robot.SPECS.UNITS[enemies[i].unit].VISION_RADIUS) {
                            return null;
                        }
                    }
                    ;
                    ...
                    if (adj != null) {
                        for (var i = 0; i < enemies.length; i++) {
                            var unit = new bc19.Position(/* get */ enemies[i].y, /* get */ enemies[i].x);
                            if (bc19.Helper.DistanceSquared(adj, unit) <= robot.SPECS.UNITS[enemies[i].unit].VISION_RADIUS) {
                                return null;
                            }
                        }
                        ;
                ...
                if (robot.location.equals(robot.previousLocation)) {
                    lowestPos = MovingRobot.LowestOnPathInMoveRange(robot, path, goal, robot.tileMovementRange - 1, robot.movementRange - 3);
                    return robot.move(lowestPos.x - robot.me.x, lowestPos.y - robot.me.y);
                }
                return null;
            }
        ...
        MovingRobot.LowestOnPathInMoveRange = function (robot, path, goal, tileMoveRange, moveRange) {
            ...
            for (var i = 0; i < validPositions.length; i++) {
                ...
                if (path[possible.y][possible.x] !== 999 && path[possible.y][possible.x] > 0 && !possible.equals(robot.location) && !possible.equals(robot.previousLocation)) {
        ...
        MovingRobot.prototype.ClosestEnemyCastle = function (robot, maps, castles) {
            var lowest = Number.MAX_VALUE;
            var output = null;
            for (var i = 0; i < castles.length; i++) {
                var flood = this.GetOrCreateMap(robot, maps, /* get */ castles[i], true);
        ...
        MovingRobot.UpdateBattleStatus = function (robot, enemies, enemy) {
            ...
            if (battleCry != null) {
                var crossMap = new bc19.Position(robot.mapIsHorizontal ? battleCry.y : robot.me.y, robot.mapIsHorizontal ? robot.me.x : battleCry.x);
                if (bc19.Helper.ContainsPosition(enemies, battleCry)) {
                    return crossMap;
                }
                else {
                    /* add */ (enemies.push(battleCry) > 0);
                    /* add */ (enemies.push(crossMap) > 0);
                    return bc19.Helper.FindEnemyCastle(robot.map, robot.mapIsHorizontal, robot.location);
                }
            }
            return enemy;
        };
        ...
        MovingRobot.prototype.Flee = function (robot, robots) {
            var closest = bc19.Helper.closestEnemy(robot, robots);
            var dx = closest.x - robot.me.x;
            var dy = closest.y - robot.me.y;
            var opposite = new bc19.Position(robot.me.y - dy, robot.me.x - dx);
            return MovingRobot.MoveCloser(robot, opposite, false);
        };
        MovingRobot.prototype.ThreatsAround = function (robot) {
            ...
            for (var i = 0; i < robots.length; i++) {
                ...
                if (robots[i].team !== robot.ourTeam && distance <= robot.visionRange && robots[i].unit !== robot.SPECS.PILGRIM && robots[i].unit !== robot.SPECS.CHURCH) {
...
(function (bc19) {
    var Helper = (function () {
        ...
        Helper.EnemiesAround = function (robot) {
            var robots = robot.getVisibleRobots();
            for (var i = 0; i < robots.length; i++) {
                if (Helper.DistanceSquared(new bc19.Position(robots[i].y, robots[i].x), robot.location) <= robot.visionRange && robots[i].team !== robot.ourTeam) {
                    return true;
                }
            }
            ;
        ...
        Helper.HighestResourceBuildPosition = function (robot, pos) {
            var highest = -100;
            var best = null;
            for (var i = -1; i <= 1; i++) {
                for (var j = -1; j <= 1; j++) {
                    ...
                    if (Helper.inMap(robot.map, adjacent) && Helper.TileEmptyNonResource(robot, adjacent)) {
                        ...
                        for (var k = -1; k <= 1; k++) {
                            for (var l = -1; l <= 1; l++) {
                                ...
                                if (Helper.inMap(robot.map, surround) && (robot.getKarboniteMap()[surround.y][surround.x] || robot.getFuelMap()[surround.y][surround.x])) {
                                    count++;
                                }
                            }
                            ;
        ...
        Helper.PositiveOrNegativeMap = function (robot, pos) {
            if (robot.mapIsHorizontal) {
                return (pos.y < (((robot.map.length + 1) / 2 | 0))) ? true : false;
            }
            else {
                return (pos.x > (((robot.map[0].length + 1) / 2 | 0))) ? true : false;
            }
        };
...
(function (bc19) {
    var StationairyRobot = (function () {
        ...
        StationairyRobot.ResourcesAround = function (robot, tileRadius) {
            var numResources = 0;
...
(function (bc19) {
    var Crusader = (function (_super) {
        ...
        function Crusader(robot) {
            ...
            _this.rushPosition = null;
            _this.fortCount = 0;
            _this.crossMapTarget = null;
            _this.robot = robot;
            return _this;
        }
        Crusader.prototype.Execute = function () {
            ...
            if (this.robot.me.turn === 1) {
                ...
                if (this.parent.signal >= 49152) {
                    var x = this.parent.signal & 63;
                    this.parent.signal >>= 6;
                    var y = this.parent.signal & 63;
                    this.rushPosition = new bc19.Position(y, x);
                }
            ...
            if (this.parent.unit === this.robot.SPECS.CASTLE) {
                this.robot.castleTalk(192 + (this.robot.mapIsHorizontal ? this.robot.me.x : this.robot.me.y));
            }
            else {
                this.robot.castleTalk(128 + (this.robot.mapIsHorizontal ? this.robot.me.x : this.robot.me.y));
            }
            if (bc19.MovingRobot.UpdateBattleStatus(this.robot, this.enemyCastleLocations, this.crossMapTarget) !== this.crossMapTarget) {
                this.crossMapTarget = bc19.MovingRobot.UpdateBattleStatus(this.robot, this.enemyCastleLocations, this.crossMapTarget);
            }
            var invader = this.ListenForDefense(this.robot);
            if (bc19.Helper.Have(this.robot, 0, 50) && this.robot.currentHealth < this.robot.previousHealth && this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.CRUSADER, this.robot.SPECS.PREACHER, this.robot.SPECS.PROPHET]).length === 0) {
                var x = this.robot.mapIsHorizontal ? this.parentLocation.x : bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.parentLocation).x;
                var y = this.robot.mapIsHorizontal ? bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.parentLocation).y : this.parentLocation.y;
                var otherSide = new bc19.Position(y, x);
                return bc19.MovingRobot.FloodPathing(this.robot, this.GetOrCreateMap(this.robot, this.routesToEnemies, otherSide, false), otherSide, false, ([]));
            }
            if (this.rushPosition != null && bc19.Helper.DistanceSquared(this.rushPosition, this.robot.location) <= 4) {
                this.rushPosition = null;
            }
            if (this.rushPosition != null) {
                var preachers = this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.PREACHER]);
                var pathingMap = this.GetOrCreateMap(this.robot, this.routesToEnemies, this.rushPosition, true);
                if (preachers.length > 0) {
                    pathingMap = this.BlackOutPreacherPaths(pathingMap, preachers);
                }
                return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, this.rushPosition, false, preachers);
            }
            if (this.initialized && bc19.Helper.Have(this.robot, 0, 325)) {
                if (this.crossMapTarget == null && !this.Fortified(this.robot, this.robot.location) && !this.manualFort) {
                    this.fortCount++;
                ...
                else if (this.crossMapTarget != null) {
                    var rushTime = true;
                    for (var i = 0; i < this.enemyCastleLocations.length; i++) {
                        this.robot.log("Castle/ Target " + this.enemyCastleLocations[i]);
                    }
                    ;
                    this.CastleDown(this.robot, this.enemyCastleLocations, this.routesToEnemies);
                    var preachers = this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.PREACHER]);
                    if (bc19.Helper.ContainsPosition(this.enemyCastleLocations, this.crossMapTarget)) {
                        if (bc19.Helper.DistanceSquared(this.robot.location, this.crossMapTarget) <= 196) {
                            rushTime = false;
                        }
                        var pathingMap = this.GetOrCreateMap(this.robot, this.routesToEnemies, this.crossMapTarget, true);
                        if (preachers.length > 0) {
                            pathingMap = this.BlackOutPreacherPaths(pathingMap, preachers);
                        }
                        return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, this.crossMapTarget, rushTime, preachers);
                    }
                    else {
                        var closestEnemyCastle = this.ClosestEnemyCastle(this.robot, this.routesToEnemies, this.enemyCastleLocations);
                        this.robot.log("Castle  : " + closestEnemyCastle);
                        ...
                        return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, closestEnemyCastle, rushTime, preachers);
                    }
                }
            }
            return null;
    ...
    var Pilgrim = (function (_super) {
        ...
        Pilgrim.prototype.Execute = function () {
            ...
            if (this.robot.me.turn === 1) {
                ...
                if (spawn.unit === this.robot.SPECS.CASTLE || spawn.signal === 65535) {
                    this.ourSide = true;
                }
                this.Initialize();
                var resources = bc19.StationairyRobot.ResourcesAround(this.robot, 5);
                var pilgrims = bc19.StationairyRobot.UnitAround(this.robot, this.robot.location, 5, this.robot.SPECS.PILGRIM);
                if (resources < pilgrims && spawn.unit === this.robot.SPECS.CHURCH) {
                    this.depotNum = 10;
                    this.churchLocation = bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.robot.location);
                }
            }
            if (this.depotNum > 0) {
            ...
            if (bc19.Helper.EnemiesAround(this.robot)) {
                this.robot.log("IMAFIMASIFMIASMFIASMIFMASIFIFS");
                if (this.state === bc19.PilgrimState.GoingToResource && bc19.Helper.Have(this.robot, 50, 250)) {
                    if (bc19.StationairyRobot.UnitAround(this.robot, this.robot.location, 3, this.robot.SPECS.CASTLE) === 0 && bc19.StationairyRobot.UnitAround(this.robot, this.robot.location, 3, this.robot.SPECS.CHURCH) === 0) {
                        var random = bc19.StationairyRobot.RandomAdjacentTowardsEnemy(this.robot, bc19.Helper.closestEnemy(this.robot, bc19.Helper.EnemiesWithin(this.robot, this.robot.visionRange)));
                        if (this.ourSide && !this.OnOppositeSide()) {
                            this.robot.signal(65535, 3);
                        }
                ...
                if (this.ThreatsAround(this.robot)) {
                    if (bc19.StationairyRobot.UnitAround(this.robot, this.robot.location, 6, this.robot.SPECS.CASTLE) > 0 || bc19.StationairyRobot.UnitAround(this.robot, this.robot.location, 6, this.robot.SPECS.CHURCH) > 0) {
                        this.state = bc19.PilgrimState.Returning;
                    }
                    var closeEnemies = bc19.Helper.EnemiesWithin(this.robot, this.robot.visionRange);
                    return this.Flee(this.robot, closeEnemies);
                }
            }
        ...
        Pilgrim.prototype.ReturnToDropOff = function () {
            ...
            if (this.GetOrCreateMap(this.robot, this.allRoutes, dropOff, true)[this.robot.location.y][this.robot.location.x] < 25) {
                ...
                var pathingMap = this.GetOrCreateMap(this.robot, this.allRoutes, dropOff, false);
                if (atkers.length > 0) {
                    pathingMap = this.BlackOutPaths(this.robot, pathingMap, atkers);
                }
                return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, dropOff, false, atkers);
            }
        ...
        Pilgrim.prototype.GetNearestResource = function (start) {
            ...
            ;
            if (this.robot.karbonite < 10) {
                chosenLocations = this.karbLocations;
                for (var i = 0; i < chosenLocations.length; i++) {
                    var pos = chosenLocations[i];
                    var distance = bc19.Helper.DistanceSquared(pos, start);
                    if (this.occupiedResources[pos.y][pos.x] === 0 && distance < lowest) {
        ...
        Pilgrim.prototype.GoToMine = function () {
            ...
            if (this.depotNum >= 0) {
                ...
                if (bc19.Helper.DistanceSquared(this.robot.location, this.churchLocation) <= 16) {
                    if (this.robot.getKarboniteMap()[this.robot.me.y][this.robot.me.x] || (this.robot.getFuelMap()[this.robot.me.y][this.robot.me.x] && this.robot.karbonite >= 10)) {
                        this.state = bc19.PilgrimState.Mining;
                        return this.robot.mine();
                    }
                    else {
                        var atkers = this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.PREACHER, this.robot.SPECS.PROPHET, this.robot.SPECS.CRUSADER]);
                        var pathingMap = this.GetOrCreateMap(this.robot, this.allRoutes, nearestToChurch, false);
                        if (atkers.length > 0) {
                            pathingMap = this.BlackOutPaths(this.robot, pathingMap, atkers);
                        }
                        return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, nearestToChurch, false, atkers);
                    }
                }
                else {
                    var atkers = this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.PREACHER, this.robot.SPECS.PROPHET, this.robot.SPECS.CRUSADER]);
                    var pathingMap = this.GetOrCreateMap(this.robot, this.allRoutes, this.churchLocation, false);
                    if (atkers.length > 0) {
                        pathingMap = this.BlackOutPaths(this.robot, pathingMap, atkers);
                    }
                    return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, this.churchLocation, false, atkers);
                }
            }
            else {
                if (this.robot.getKarboniteMap()[this.robot.me.y][this.robot.me.x] || this.robot.getFuelMap()[this.robot.me.y][this.robot.me.x]) {
                    ...
                    var atkers = this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.PREACHER, this.robot.SPECS.PROPHET, this.robot.SPECS.CRUSADER]);
                    var pathingMap = this.GetOrCreateMap(this.robot, this.allRoutes, nearestInGeneral, false);
                    if (atkers.length > 0) {
                        pathingMap = this.BlackOutPaths(this.robot, pathingMap, atkers);
                    }
                    return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, nearestInGeneral, false, atkers);
                }
            }
        };
        Pilgrim.prototype.Mining = function () {
            if (bc19.Helper.Have(this.robot, 50, 250) && bc19.StationairyRobot.UnitAround(this.robot, this.robot.location, 6, this.robot.SPECS.CASTLE) === 0 && bc19.Helper.DistanceSquared(this.robot.location, this.GetNearestDropOff()) > 5) {
                var random = bc19.Helper.HighestResourceBuildPosition(this.robot, this.robot.location);
                if (this.ourSide && !this.OnOppositeSide()) {
                    this.robot.signal(65535, 3);
                }
                return this.robot.buildUnit(this.robot.SPECS.CHURCH, random.x - this.robot.me.x, random.y - this.robot.me.y);
            ...
            else if (!this.robot.getKarboniteMap()[this.robot.me.y][this.robot.me.x] && !(this.robot.getFuelMap()[this.robot.me.y][this.robot.me.x])) {
                this.state = bc19.PilgrimState.Returning;
            }
            else
                return this.robot.mine();
        ...
        Pilgrim.prototype.OnOppositeSide = function () {
            if (this.robot.mapIsHorizontal) {
                if (bc19.Helper.sign(((this.robot.map.length / 2 | 0)) - this.robot.me.y) !== bc19.Helper.sign(((this.robot.map.length / 2 | 0)) - this.spawnLocation.y)) {
                    return true;
                }
                else
                    return false;
                ...
                if (bc19.Helper.sign(((this.robot.map.length / 2 | 0)) - this.robot.me.x) !== bc19.Helper.sign(((this.robot.map.length / 2 | 0)) - this.spawnLocation.x)) {
                    return true;
                }
                else
                    return false;
...
(function (bc19) {
    var Preacher = (function (_super) {
        ...
        Preacher.prototype.Execute = function () {
            ...
            if (bc19.MovingRobot.UpdateBattleStatus(this.robot, this.enemyCastleLocations, this.changeCheck) !== this.changeCheck) {
                this.changeCheck = bc19.MovingRobot.UpdateBattleStatus(this.robot, this.enemyCastleLocations, this.changeCheck);
            }
            var invader = this.ListenForDefense(this.robot);
            ...
            if (bc19.Helper.Have(this.robot, 0, 50) && this.robot.currentHealth < this.robot.previousHealth && this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.CRUSADER, this.robot.SPECS.PREACHER, this.robot.SPECS.PROPHET]).length === 0) {
                var otherSide = bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.robot.location);
                return bc19.MovingRobot.FloodPathing(this.robot, this.GetOrCreateMap(this.robot, this.routesToEnemies, otherSide, false), otherSide, false, ([]));
            }
            if (this.initialized && bc19.Helper.Have(this.robot, 0, 325)) {
                this.robot.log("Fortified " + !this.Fortified(this.robot, this.robot.location));
                if (this.changeCheck == null && !this.Fortified(this.robot, this.robot.location) && !this.manualFort) {
                    this.fortCount++;
                    if (this.fortCount > 10 && bc19.Helper.DistanceSquared(this.parentLocation, this.robot.location) <= 100) {
                        this.manualFort = true;
                    }
                    var valid = this.GetValidFortifiedPositions(this.robot, this.parentLocation);
                    if (valid.length > 0) {
    ...
    var Prophet = (function (_super) {
        ...
        Prophet.prototype.Execute = function () {
            ...
            if (bc19.Helper.EnemiesAround(this.robot)) {
                ...
                else if (bc19.Helper.Have(this.robot, 0, 75)) {
            ...
            if (this.initialized && bc19.Helper.Have(this.robot, 0, 325)) {
                if (this.changeCheck == null && !this.Fortified(this.robot, this.robot.location) && !this.manualFort) {
                    ...
                    var crusaders = this.EnemiesOfTypeInVision(this.robot, [this.robot.SPECS.CRUSADER]);
                    this.CastleDown(this.robot, this.enemyCastleLocations, this.routesToEnemies);
                    var closestEnemyCastle = this.ClosestEnemyCastle(this.robot, this.routesToEnemies, this.enemyCastleLocations);
                    var pathingMap = this.GetOrCreateMap(this.robot, this.routesToEnemies, closestEnemyCastle, true);
                    if (crusaders.length > 0) {
                        pathingMap = this.BlackOutPaths(this.robot, pathingMap, crusaders);
                    }
                    return bc19.MovingRobot.FloodPathing(this.robot, pathingMap, closestEnemyCastle, true, crusaders);
                }
            }
            return null;
        };
...
(function (bc19) {
    var Castle = (function (_super) {
        ...
        function Castle(robot) {
            ...
            _this.crusaderNumbers = null;
            _this.ourCrusaders = 0;
            _this.totalCrusaders = 0;
            _this.castleCrusaders = 0;
            _this.imMostCentral = false;
            _this.robot = robot;
            return _this;
        }
        Castle.prototype.Execute = function () {
            this.robot.log("TTTTTTTTTTTTTTTTurn " + this.robot.me.turn + "  " + this.robot.fuel);
            var output = null;
            var signal = -1;
            var signalRadius = 0;
            if (!this.initialized) {
            ...
            if (this.robot.me.turn > 4 && this.initialized && bc19.Helper.Have(this.robot, 50 + this.robot.SPECS.UNITS[this.spawnOrder[this.positionInSpawnOrder]].CONSTRUCTION_KARBONITE, 500) && this.ourCrusaders <= (this.castleCrusaders / this.numCastles | 0)) {
            ...
            this.UpdateDepots();
            var closest = this.DepotClosestToCenter(this.MyCenterDepots());
            if (((closest == null && this.robot.karbonite > 35) || this.robot.me.turn > 6) && (bc19.Helper.Have(this.robot, 20, 125) || this.starterPilgrims < 4)) {
                this.UpdateDepots();
                var pilgrimPosition = this.DepotClosestToCenter(this.AvailableDepots());
                if (pilgrimPosition != null) {
                    var random = bc19.Helper.HighestResourceBuildPosition(this.robot, new bc19.Position(this.robot.me.y, this.robot.me.x));
            ...
            else if (((closest == null && this.robot.karbonite > 35) || this.robot.me.turn > 6) && this.NumPilgrims() < 2) {
                this.UpdateDepots();
                var pilgrimPosition = this.DepotClosestToCenter(this.AvailableDepots());
                if (pilgrimPosition != null) {
                    var random = bc19.Helper.HighestResourceBuildPosition(this.robot, new bc19.Position(this.robot.me.y, this.robot.me.x));
            ...
            var resources = bc19.StationairyRobot.ResourcesAround(this.robot, 3);
            var pilgrims = this.CastlePilgrims();
            this.robot.log((closest == null && this.robot.karbonite > 35) + "   Bool");
            this.robot.log(closest + "  closest");
            if ((this.robot.me.turn < ((this.numCastles === 1) ? 6 : 2) || (closest == null && this.robot.karbonite > 35) || this.robot.me.turn > 6) && resources > pilgrims) {
                var buildHere = bc19.Helper.HighestResourceBuildPosition(this.robot, this.robot.location);
            ...
            signalRadius = atkSignal === -1 ? 3 : this.FindSignalRadius();
            if (this.robot.me.turn === 3 && this.numCastles === 1) {
            ...
            if (closest != null && this.robot.me.turn <= 6) {
                this.robot.log("in loop");
                if (this.robot.me.turn === (this.numCastles === 1 ? 1 : 3) + (this.robot.me.turn > 4 ? 2 : 0)) {
                    var churchAcross = bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, closest);
                    var validNear = bc19.Helper.RandomAdjacentNonResource(this.robot, churchAcross);
                    var buildHere = bc19.StationairyRobot.RandomAdjacentTowardsEnemy(this.robot, churchAcross);
                    if (buildHere != null) {
                        signal = this.CreateAttackSignal(validNear, 12);
                        this.starterPilgrims++;
                        output = this.robot.buildUnit(this.robot.SPECS.CRUSADER, buildHere.x - this.robot.me.x, buildHere.y - this.robot.me.y);
                    }
                }
                if (this.robot.me.turn === (this.numCastles === 1 ? 2 : 4) + (this.robot.me.turn > 5 ? 2 : 0)) {
                    var buildHere = bc19.StationairyRobot.RandomAdjacentTowardsEnemy(this.robot, bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.robot.location));
                    if (buildHere != null) {
                        this.UpdateDepots();
                        signal = this.SignalToPilgrim(closest);
                        this.starterPilgrims++;
                        output = this.robot.buildUnit(this.robot.SPECS.PILGRIM, buildHere.x - this.robot.me.x, buildHere.y - this.robot.me.y);
                    }
                }
                        ...
                        var pos = array126[index125];
                        {
                            ...
                            /* put */ (function (m, k, v) { if (m.entries == null)
                                ...
                                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                    m.entries[i].value = v;
                                    return;
                                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(this.crusaderNumbers, pos, 0);
                        }
                    }
                }
            }
        ...
        Castle.prototype.InitializeVariables = function () {
            this.spawnOrder = [this.robot.SPECS.PROPHET, this.robot.SPECS.CRUSADER, this.robot.SPECS.CRUSADER, this.robot.SPECS.PROPHET, this.robot.SPECS.CRUSADER, this.robot.SPECS.PREACHER];
        ...
        Castle.prototype.SignalAttack = function () {
            ...
            if (this.RushParameters(otherCastlesCry)) {
        ...
        Castle.prototype.RushParameters = function (other) {
            if (other != null) {
                return false;
            }
            if (this.robot.me.turn !== 300 && this.robot.me.turn !== 500 && this.robot.me.turn !== 700 && this.robot.me.turn !== 900) {
                return false;
            }
            var distance = (Math.sqrt(bc19.Helper.DistanceSquared(this.robot.location, bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.robot.location))) | 0);
            if (!bc19.Helper.Have(this.robot, 0, ((Math.fround(distance * 1.1)) | 0) * this.totalCrusaders)) {
                return false;
            }
            var twoThirds = ((this.robot.fuel * 2) / 3 | 0);
            if (this.totalCrusaders * 75 < twoThirds) {
                return false;
            }
            return true;
        };
        Castle.prototype.FindSignalRadius = function () {
            var farthest = 0;
            var corners = [new bc19.Position(0, 0), new bc19.Position(0, this.robot.map.length), new bc19.Position(this.robot.map.length, 0), new bc19.Position(this.robot.map.length, this.robot.map.length)];
            for (var i = 0; i < corners.length; i++) {
                var distance = (bc19.Helper.DistanceSquared(this.robot.location, corners[i]) | 0);
        ...
        Castle.prototype.ReadCrusaders = function () {
            var robots = this.robot.getVisibleRobots();
            this.totalCrusaders = 0;
            this.ourCrusaders = 0;
            this.castleCrusaders = 0;
            for (var i = 0; i < this.allyCastlePositions.length; i++) {
                /* put */ (function (m, k, v) { if (m.entries == null)
                    ...
                    if (m.entries[i_2].key.equals != null && m.entries[i_2].key.equals(k) || m.entries[i_2].key === k) {
                        ...
                        return;
                    } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(this.crusaderNumbers, /* get */ this.allyCastlePositions[i], 0);
            }
            ;
            for (var i = 0; i < robots.length; i++) {
                var r = robots[i];
                if (r.castle_talk >= 128 && r.castle_talk < 192) {
                    ...
                    var closest = null;
                    {
                        var array128 = (function (m) { var r = []; if (m.entries == null)
                            ...
                            r.push(m.entries[i_3].key); return r; })(this.crusaderNumbers);
                        for (var index127 = 0; index127 < array128.length; index127++) {
                            var pos = array128[index127];
                            {
                                var distance = bc19.Helper.abs((r.castle_talk - 192) - (this.robot.mapIsHorizontal ? pos.x : pos.y));
                    ...
                    this.totalCrusaders++;
                    this.castleCrusaders++;
                    if (this.robot.location.equals(closest)) {
                        this.ourCrusaders++;
                    }
                    /* put */ (function (m, k, v) { if (m.entries == null)
                        ...
                        if (m.entries[i_4].key.equals != null && m.entries[i_4].key.equals(k) || m.entries[i_4].key === k) {
                            ...
                            return;
                        } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(this.crusaderNumbers, closest, /* get */ (function (m, k) { if (m.entries == null)
                        ...
                        } return null; })(this.crusaderNumbers, closest) + 1);
                }
            }
            ;
        };
        Castle.prototype.AvailableDepots = function () {
            var available = ([]);
        ...
        Castle.prototype.MyCenterDepots = function () {
            var mine = ([]);
            for (var i = 0; i < this.churchLocations.length; i++) {
                var length_1 = 0;
                if (bc19.Helper.DistanceSquared(this.robot.location, bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.robot.location)) > bc19.Helper.DistanceSquared(/* get */ this.churchLocations[i], bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, /* get */ this.churchLocations[i]))) {
                    if (this.robot.mapIsHorizontal) {
                        length_1 = bc19.Helper.abs(/* get */ this.churchLocations[i].y - (((this.robot.map.length + 1) / 2 | 0)));
                    }
                    else {
                        length_1 = bc19.Helper.abs(/* get */ this.churchLocations[i].x - (((this.robot.map.length + 1) / 2 | 0)));
                    }
                    if (length_1 <= 7) {
                        var lowest = (bc19.Helper.DistanceSquared(this.robot.location, /* get */ this.churchLocations[i]) | 0);
                        var closest = this.robot.location;
                        for (var j = 0; j < this.allyCastlePositions.length; j++) {
                            var distance = (bc19.Helper.DistanceSquared(/* get */ this.allyCastlePositions[j], /* get */ this.churchLocations[i]) | 0);
                            if (distance < lowest) {
                                lowest = distance;
                                closest = this.allyCastlePositions[j];
                            }
                        }
                        ;
                        if (closest.equals(this.robot.location) && this.resourceDepots[i] < 0) {
                            /* add */ (mine.push(/* get */ this.churchLocations[i]) > 0);
                        }
                    }
                }
            }
            ;
            this.robot.log("the size " + mine.length);
            return mine;
        };
        Castle.prototype.DepotClosestToCenter = function (depotLocations) {
            var lowest = 128;
            var closest = null;
            for (var i = 0; i < depotLocations.length; i++) {
                var current = Number.MAX_VALUE;
                if (this.robot.mapIsHorizontal) {
                    current = bc19.Helper.abs(/* get */ depotLocations[i].y - (((this.robot.map.length + 1) / 2 | 0)));
                }
                else {
                    current = bc19.Helper.abs(/* get */ depotLocations[i].x - (((this.robot.map.length + 1) / 2 | 0)));
                }
                if (current < lowest) {
                    lowest = current;
                    closest = depotLocations[i];
                }
            }
            ;
            return closest;
...
(function (bc19) {
    var Church = (function (_super) {
        ...
        function Church(robot) {
            ...
            _this.spawnOrder = null;
            _this.built = 0;
            _this.crusadersTurn = false;
            _this.first = false;
            _this.distanceMiddle = 0;
            _this.last = 0;
        ...
        Church.prototype.Execute = function () {
            ...
            if (this.robot.me.turn === 1) {
                ...
                this.ourSide = false;
                this.GetSideSignal();
                this.crusadersTurn = true;
                this.first = true;
                this.last = 0;
                this.building = false;
                if (this.robot.mapIsHorizontal) {
                    this.distanceMiddle = bc19.Helper.abs(this.robot.me.y - (((this.robot.map.length + 1) / 2 | 0)));
                }
                else {
                    this.distanceMiddle = bc19.Helper.abs(this.robot.me.x - (((this.robot.map.length + 1) / 2 | 0)));
                }
            }
            var output = null;
            var signal = -1;
            this.positionInSpawnOrder = this.positionInSpawnOrder === this.spawnOrder.length ? 0 : this.positionInSpawnOrder;
            if (bc19.Helper.Have(this.robot, 80 + this.robot.SPECS.UNITS[this.spawnOrder[this.positionInSpawnOrder]].CONSTRUCTION_KARBONITE, 500)) {
                var buildHere = bc19.Helper.RandomAdjacentNonResource(this.robot, this.robot.location);
                if (buildHere != null) {
                    output = this.robot.buildUnit(this.spawnOrder[this.positionInSpawnOrder], buildHere.x - this.robot.me.x, buildHere.y - this.robot.me.y);
                    this.positionInSpawnOrder++;
            ...
            var resources = bc19.StationairyRobot.ResourcesAround(this.robot, 10);
            var pilgrims = bc19.StationairyRobot.UnitAround(this.robot, this.robot.location, 10, this.robot.SPECS.PILGRIM);
            if (this.distanceMiddle >= 5 && this.ourSide) {
                var timing = (this.first || this.robot.me.turn - this.last > 150);
                var builtUp = this.built === resources;
                var roundWorthy = timing && builtUp;
                var acceptable = this.building || roundWorthy;
                this.robot.log("Enemies around " + !bc19.Helper.EnemiesAround(this.robot));
                this.robot.log("Built : " + this.built + "resources = " + resources);
                this.robot.log("timing " + timing);
                this.robot.log("Built ip " + builtUp);
                this.robot.log("round worthy " + roundWorthy);
                this.robot.log("Sum total " + acceptable);
                if (bc19.Helper.Have(this.robot, 15, 100) && !bc19.Helper.EnemiesAround(this.robot) && acceptable) {
                    if (this.built === resources) {
                        if (this.crusadersTurn) {
                            this.building = true;
                            this.robot.log("Crusader was born");
                            var churchAcross = bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.robot.location);
                            var validNear = bc19.Helper.RandomAdjacentNonResource(this.robot, churchAcross);
                            var buildHere = bc19.StationairyRobot.RandomAdjacentTowardsEnemy(this.robot, churchAcross);
                            if (buildHere != null) {
                                signal = this.CreateAttackSignal(validNear, 12);
                                output = this.robot.buildUnit(this.robot.SPECS.CRUSADER, buildHere.x - this.robot.me.x, buildHere.y - this.robot.me.y);
                                this.crusadersTurn = false;
                            }
                        }
                        else if (!this.crusadersTurn) {
                            this.robot.log("Pilgrim was born");
                            this.building = false;
                            var buildHere = bc19.StationairyRobot.RandomAdjacentTowardsEnemy(this.robot, bc19.Helper.FindEnemyCastle(this.robot.map, this.robot.mapIsHorizontal, this.robot.location));
                            if (buildHere != null) {
                                if (this.ourSide) {
        ...
        Church.prototype.GetSideSignal = function () {
            var robots = this.robot.getVisibleRobots();
            for (var i = 0; i < robots.length; i++) {
                if (bc19.Helper.DistanceSquared(this.robot.location, new bc19.Position(robots[i].y, robots[i].x)) <= 3) {
                    this.robot.log("My Neightbors : " + new bc19.Position(robots[i].y, robots[i].x));
                    this.robot.log("The signal " + robots[i].signal);
                    this.robot.log("bool " + (robots[i].signal === 65535));
                    if (robots[i].signal === 65535) {
                        this.robot.log("WAS TRUE");
                        this.ourSide = true;
                    }
                }
            }
...
(function (bc19) {
    var MyRobot = (function (_super) {
        ...
        MyRobot.prototype.Setup = function () {
            ...
            this.positiveSide = bc19.Helper.PositiveOrNegativeMap(this, this.location);
